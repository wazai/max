...
class Covar(object):
    ...
    def _get_cov_seq(self):
        ...
        self.ex_post['cor'] = np.array([util.ex_post_cor(self.ret, self.window, x) for x in self.ret.index])
        self.ex_post['vol'] = np.array([util.ex_post_vol(self.ret, self.window, x) for x in self.ret.index])
        self.ex_post['cov'] = np.array([util.ex_post_cov(self.ret, self.window, x) for x in self.ret.index])
    ...
    def plot_fit(self, variable, i, j=None):
        ...
        util.plot_fit(ex_post_series, index=self.ret.index[self.window:], y_label=variable, legend=[self.name])
