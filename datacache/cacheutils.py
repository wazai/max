...
def get_index_composition(index):
    ...
    return pd.read_csv(filename, dtype={'code': object})

def get_code_list(index_list):
    dat = pd.DataFrame()
    for index in index_list:
        dat = dat.append(get_index_composition(index))
    return list(set(dat['code']))

def get_prevclose(dt):
    dt['prevclose'] = dt.close.shift(1)
    return dt

def rolling_operation(func, windowlen, col, newcol):
    def f(dt):
        dt[newcol] = func(arg=dt[col], window=windowlen, min_periods=windowlen).tolist()
        return dt
    return f
